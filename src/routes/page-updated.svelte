  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        // Optional: Still open email client for immediate contact
        const subject = encodeURIComponent('Prayer Commission Request Submitted');
        const body = encodeURIComponent(`Thank you for your prayer commission request.

Your request has been submitted with ID: ${result.id}
...
We will contact you shortly with payment instructions.`);
      ...
      errorMessage = 'Network error. Please try again or contact us directly.';
    } finally {
      isSubmitting = false;
    }
  }
